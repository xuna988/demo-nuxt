...
export const actions = {
  nuxtServerInit (store, {params, route, req}) {
    ...
    initAppData = [
     ...
     store.dispatch('loadSideList'),
      store.dispatch('loadDouguoIndex'),
      store.dispatch('loadDouguoListIndex')
    ]
  ...
  async loadDouguoListIndex ({commit},params = {}) {
    return Service.get(`/api/douguoIndexListApi?start=1`)
      .then(res => {
        console.log("真实数据真实数据")
  ...
  async loadDouguoIndex ({commit}, params = {}) {
    return Service.get(`/api/douguoIndexApi?start=10`)
      .then(res => {
  ...
  loadTag ({commit}) {
    return Service.get(`/api/oftenTag?num=1`)
      .then(res => {
        console.log("感兴趣的事情如下：")
        console.log(res.data)
        // console.log(res)
        // console.log("热门标签123333")
        // let tagList = [{
        //   id:'aaaa',
        //   content:'bbbb',
        //   order_id:"cccc"
